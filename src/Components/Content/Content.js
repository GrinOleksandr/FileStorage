...
import MyFiles from "../MyFiles/MyFiles.js";
...
class Content extends Component {
    constructor(props){
        ...
        this.dropZoneOnDragLeave = this.dropZoneOnDragLeave.bind(this);
    ...
    //     e.stopPropagation();
    //     e.preventDefault();
    //     let dx = e.pageX - this.DropZone.getBoundingClientRect().left + pageXOffset;
    //     let dy = e.pageY - this.DropZone.getBoundingClientRect().top + pageYOffset;
    //     if ((dx < 0) || (dx > this.DropZone.clientWidth) || (dy < 0) || (dy > this.DropZone.clientHeight)) {
    //         this.DropZone.classList.remove('dragover');
    //     }
    // }
    ...
    <div id='content'>
        ...
        onDragOver={this.dropZoneOnDragOver} onDragLeave={this.dropZoneOnDragLeave}>
            <img id="upload-image" src="img/upload.png" />
                <div>
                    <input id="file-input" type="file" name="fileInput" multiple allowdirs onClick={this.fileInputHandler} />
        ...
        <ul id='files-list'>
            {this.state.MyFiles.map((file, idx) => <MyFiles file={file} key={idx} />)}
        </ul>
    </div>
</div>
